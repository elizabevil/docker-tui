...
- 第一阶段实现记录（2026-07-20）:
  1. 已建立 `ActionSpec / BindingSet / Context / Resolver`，运行时优先级固定为 `Mode > View > Surface > App`。
  2. `config.Keymap` 已作为动作级覆盖输入；空配置回退注册表默认绑定，显式配置替换对应动作默认绑定。
  3. 主资源页、镜像与 Compose 容器子视图、Detail、Logs、Mark、Help 已接入语义动作解析。
  4. Filter 与 Command 的文本编辑已抽离为共享输入编辑器，支持 shell 风格移动和删除，并保持 rune 安全。
  5. 资源级删除动作优先于旧通用 `delete` fallback；覆盖资源删除键后，旧通用键不会旁路触发同一资源删除。
  6. 本阶段尚未把 Compose 专有动作、左右焦点、排序、复制、Debug、Export 等未进入 `KeymapConfig` 的动作开放为用户配置项。
...
- 第一阶段实现记录（2026-07-20）:
  1. 默认配置已统一为 `Ctrl+S / Ctrl+R / Ctrl+K / Ctrl+P / Ctrl+D`，Help 同时默认支持 `? / F1`。
  2. `DefaultKeyMapping()` 改为从统一动作注册表派生，仅保留兼容 API，不再维护独立默认表。
  3. Help、Footer 的可配置动作键位从当前有效绑定投影；模式页的导航与关闭提示也使用有效绑定。
  4. 已增加默认配置与注册表一致性测试，以及自定义 Help、导航、返回、资源删除覆盖的运行时测试。
  5. Help / Footer 的分组与动作描述仍位于 UI 投影层；动作、默认绑定和上下文可用性由动作注册表负责。
