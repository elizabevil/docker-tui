	...
	go func() {
		...
		for {
			...
			case msg, ok := <-msgCh:
				...
				if !runtimeapi.SendEventItem(ctx, out, runtimeapi.EventItem{Event: mapDockerEvent(msg)}) {
					return
				}
			case err, ok := <-errCh:
				if !ok {
				...
				item := runtimeapi.EventItem{Error: runtimeapi.MapRuntimeError(err, runtimeapi.Operation(runtimeapi.ResourceEvent, "subscribe"), runtimeapi.ResourceRef{}, runtimeapi.Docker)}
				if !runtimeapi.SendEventItem(ctx, out, item) {
					return
				}
			}
		}
