...
func TestMapRuntimeErrorClassifiesDriverErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := runtimeapi.MapRuntimeError(test.err, "container.start", ref, runtimeapi.Podman)
...
func TestMapRuntimeErrorDoesNotMutateExistingError(t *testing.T) {
	...
	mapped := runtimeapi.MapRuntimeError(original, "container.pause", runtimeapi.ResourceRef{Type: runtimeapi.ResourceContainer, ID: "id"}, runtimeapi.Docker)
