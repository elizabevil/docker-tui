...
func handleTopKey(key string, m *state.AppModel) (*state.AppModel, tea.Cmd) {
	...
	case keys.KeyDown, keys.KeyJ:
		m.Processes.Move(1)
	case keys.KeyR:
		if m.Connection.Engine != nil && m.Processes.ContainerID != "" && !m.Processes.Loading {
			m.Processes.Loading = true
			return m, fetchContainerProcesses(m.Connection.Engine.Containers(), m.Processes.ContainerID)
		}
	}
	return m, nil
}
