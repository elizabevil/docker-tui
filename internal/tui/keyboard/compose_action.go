...
// each via ImageTransferService (R08-05). Untagged images (no
// `com.docker.compose.image` label) are filtered out — same predicate
// as the `compose_tagged` Action Bar requirement.
func doComposePush(m *state.AppModel) (*state.AppModel, tea.Cmd) {
	...
	containers := composeProjectContainers(m, project)
	if len(containers) == 0 {
		ShowToastWarn(m, "✕ compose push failed: no containers")
		return m, nil
	}
	tagged := taggedComposeImages(containers)
	if len(tagged) == 0 {
		ShowToastWarn(m, "✕ compose push: no tagged images in project")
		return m, nil
	}
	ShowToastNow(m, fmt.Sprintf("⟳ compose push %s (%d images)", project, len(tagged)))
	return m, fetchComposePush(m.Connection.Engine, project, tagged)
}

func fetchComposePush(eng runtime.Engine, project string, images []runtime.ContainerSummary) tea.Cmd {
	return func() tea.Msg {
		var failures []error
		successes := 0
		for _, c := range images {
			tag := runtime.ComposeImageFromLabels(c)
			if tag == "" {
				continue
			}
			ch, err := eng.ImageTransfers().Run(context.Background(), runtime.ImageTransferRequest{
				Operation:   runtime.ImageTransferPush,
				Source:      tag,
				Destination: tag,
			})
			if err != nil {
			...
			for ev := range ch {
				if ev.Error != nil {
					failures = append(failures, fmt.Errorf("%s: %w", tag, ev.Error))
				}
				if ev.Result != nil {
					successes++
				}
		...
		return state.ComposeServicePushCompleted{
			Project: project,
			Success: successes,
...
// taggedComposeImages returns the unique image references for
// containers that carry the `com.docker.compose.image` label.
func taggedComposeImages(containers []runtime.ContainerSummary) []runtime.ContainerSummary {
	...
	for _, c := range containers {
		...
		tag := c.Labels[runtime.ComposeLabelImage]
		if tag == "" {
			continue
		}
		if _, ok := seen[tag]; ok {
